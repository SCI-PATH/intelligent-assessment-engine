...
@router.get(
    ...
    description=(
        ...
        "**Outbound Component 1:** `GET {COMPONENT_1_URL}/progress?user_id=...` "
        "(maps `g6_sci_NN` → `G6_CN`; hardcoded `G6_C8` while `C1_HTTP_LIVE=False` "
        "or on timeout/error).\n\n"
...
@router.post(
    ...
    description=(
        ...
        "- If omitted → `GET` Component 1 `/progress?user_id=` "
        "(map to `G6_Cn`; fallback `G6_C8` when `C1_HTTP_LIVE=False` or C1 fails).\n\n"
